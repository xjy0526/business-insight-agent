...
> Product Ads Extension of BusinessInsight Agent
...
本项目具有商品级广告增长决策能力，
包括主推品挖掘、Query-SKU 召回、ROI 出价守护、投放策略报告、Eval/Ablation 和 Notebook。

项目定位保持为：
面向本地生活商户的商品级广告增长决策 Agent，基于 Tool Calling 与 RAG 完成主推品挖掘、
Query-SKU 召回与 ROI 守护。

所有新增数据均为 synthetic demo data，不涉及真实平台、真实商户、真实用户或敏感业务信息。

## Product Ads Extension

系统使用 Tool Calling 与 RAG 组合：
关键数字由 SQLite 工具计算，商品级广告建议由 `product_ad_tool` 基于 synthetic demo data 生成，
RAG 文档提供抽象策略知识。
...
Trace/Eval/Fallback 让 AI 应用从“能回答”走向“可观测、可回测、可降级”。

展示关键词：`Agent` · `RAG` · `Tool Calling` · `Business Diagnosis` · `Observability` · `Evaluation` · `Ablation` · `Fallback` · `Safety` · `FastAPI` · `Docker`

## 面试官快速导览

**30 秒定位：** 这是一个“数字由工具计算、结论由证据支撑、过程可回放”的经营分析 Agent。LLM 负责理解和表达，SQLite/RAG/Reflection 分别负责确定性指标、业务规则和输出校验。

**建议 5 分钟演示：**

1. 提问“商品 P1001 最近 GMV 为什么下降？”。
2. 在结果中指出指标拆解、评论主题、活动机会和引用证据。
3. 打开 Trace 展示路由、工具调用、耗时与 Reflection 修复过程。
4. 运行一个 prompt injection 用例，再展示 Eval/Ablation 中各组件的贡献。

如果只读三个文件，建议依次查看 [Demo Cases](docs/demo_cases.md)、[Architecture](docs/architecture.md) 和 [Latest Evaluation](#latest-evaluation)。

## 文档导航

- [Architecture](docs/architecture.md)：架构、节点、工具层、RAG、Reflection、Trace、Eval、Fallback、安全设计
- [Demo Cases](docs/demo_cases.md)：6 个可直接演示的问题、工具证据和讲解要点
...
- [Product Ads Method](PRODUCT_AD_METHOD.md)：商品级广告方法、公式和实验设计
- [Final Report](docs/final_report.md)：最终项目报告正文
- [References](docs/references.md)：相关工作与参考文献链接
- [Manual Eval Notes](docs/manual_eval_notes.md)：人工小样本评审说明
...
默认 `AGENT_RUNNER=sequential`，使用项目内置轻量条件 runner。它没有额外依赖，适合本地开发、CI 和本地演示。
...
| P1001 搜索渠道点击率为什么下降？ | `traffic_analysis` | search 渠道 CTR 拆解、项目图/标题/价格竞争力解释 |
| 商品 P1001 差评集中在哪些问题？ | `review_analysis` | Review Tool 输出效果、等待时间、服务体验等主题 |
...
内置模拟数据覆盖 `products/orders/traffic/reviews/campaigns`，核心异常包括 P1001 四月 GMV 下滑、退款率升高、search CTR 下滑、差评集中在效果/等待时间/服务体验、丽人医美类目活动参与不足。
...
| --- | ---: | ---: | ---: | --- |
| `full_agent` | 0.974587 | 1.0 | 0.485606 | 完整链路 |
| `no_rag` | 0.938718 | 0.590909 | 0.480303 | 检验 RAG 对证据命中的贡献 |
| `no_reflection` | 0.965735 | 1.0 | 0.2 | 检验 Reflection 对证据修复和安全审计的贡献 |
| `no_metrics_tool` | 0.899390 | 0.977273 | 0.2 | 检验确定性工具对数字可靠性的贡献 |
| `mock_only` | 0.839828 | 0.590909 | 0.2 | 纯 mock/fallback 基线 |
