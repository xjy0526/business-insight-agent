...
- `review_analysis`：四月差评主题集中在效果不明显、等待时间长、服务体验。
- `campaign_participation`：丽人医美类目存在活动机会，P1001 活动参与不足，`risk_level=high`。
...
- `review_analysis.top_topics`：效果不达预期、等待时间慢、服务体验不舒服等主题。
- RAG evidence：`after_sales_policy.md`、`review_analysis_guide.md`。

**预期输出亮点**

报告能把退款率异常与差评主题联系起来，但不会把“差评”直接写成唯一原因；会建议继续核对售后工单、等待时间履约和商品描述。
...
- RAG evidence：`product_operation_guide.md` 中关于项目图、标题、价格利益点对 CTR 的影响。

**预期输出亮点**

报告重点不是泛泛讲“优化流量”，而是定位到 search 渠道，并给出项目图、标题、到店价、活动利益点、竞品卡片对比等操作建议。

**项目要点**

“这个 case 展示渠道拆解能力。CTR 下降不是一个总指标就能解释的，所以我把渠道拆开看 search，再让 RAG 补充运营解释。这里也能说明 Tool 和 RAG 的边界：Tool 负责算 search CTR，RAG 负责解释为什么项目图、标题、价格会影响点击。”
...
- `review_analysis.topic_distribution`：效果不达预期、等待时间慢、服务体验不舒服等主题分布。
...
“没有把评论分析完全交给 LLM，因为评论主题、差评率和样例评论都是可审计事实。这里用 Review Tool 做规则统计，LLM 只负责把结果解释成经营建议，避免编造评论内容。”
...
- `campaign_participation.eligible_campaigns`：四月丽人医美类目活动机会。
